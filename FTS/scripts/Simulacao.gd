...
func _process(delta):	
	...
	if !has_any_food():
		...
		popEnd = fishes.size()
		print(gen, ",", popInit, ",", normalSpeed, ",", mEnergy, ",", Radarsz, ",",deathR , ",", birthR, ",", popEnd)
...
func attribMean():
	var attrib_mean_list = [0, 0, 0, 0]
	
	for fish in fishes:
		attrib_mean_list[0] += fish.normalSpeed
		attrib_mean_list[1] += fish.maxEnergy
		attrib_mean_list[2] += fish.foodRadarSize
	
	normalSpeed = (attrib_mean_list[0] / fishes.size())
	mEnergy     = (attrib_mean_list[1] / fishes.size())
	Radarsz     = (attrib_mean_list[2] / fishes.size())
